...
export async function POST(request: NextRequest) {
  try {
    ...
    // For PDFs, provide helpful instructions instead of failing
    if (fileType === 'application/pdf') {
      return NextResponse.json(
        { 
          error: 'PDF parsing is currently unavailable. Please try one of these alternatives:\n\n1. Convert your PDF to a Word document (.docx) and upload that instead\n2. Copy and paste the lab results as text using the JSON upload option\n3. Use the guided form to manually enter your lab values\n4. Take a screenshot of the PDF and describe the values to the AI assistant\n\nWord documents (.docx) work perfectly with our AI parsing system!'
        },
        { status: 400 }
      );
    }
...
Only return valid JSON, no explanatory text.`,
    ...
    return NextResponse.json(
      { error: 'Unsupported file type. Please upload a Word document (.docx) or use the JSON upload option.' },
      { status: 400 }
    );
